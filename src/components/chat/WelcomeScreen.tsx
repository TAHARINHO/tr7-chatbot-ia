...
import { AIAvatar, InayaWelcomeCard } from "./Avatars";
...
const AGENT_SUGGESTIONS: Record<string, { icon: React.ReactNode; title: string; prompt: string; color: string }[]> = {
  "community-management": [
    ...
    { icon: <ZapIcon className="h-4 w-4" />, title: "Hooks viraux TikTok", prompt: "Génère 5 hooks ultra-percutants pour TikTok sur le thème de l'automatisation IA.", color: "bg-rose-50 text-rose-600 group-hover:bg-rose-100" },
  ...
  "video-motion": [
    { icon: <ZapIcon className="h-4 w-4" />, title: "Script TikTok 30s", prompt: "Écris un script complet pour un TikTok de 30 secondes présentant un outil IA.", color: "bg-purple-50 text-purple-600 group-hover:bg-purple-100" },
    ...
    { icon: <TrendingUpIcon className="h-4 w-4" />, title: "Prompt IA vidéo", prompt: "Donne-moi un prompt détaillé pour Runway pour générer une scène de présentation tech.", color: "bg-pink-50 text-pink-600 group-hover:bg-pink-100" },
  ],
  "ux-ui-design": [
    { icon: <LightbulbIcon className="h-4 w-4" />, title: "Identité visuelle", prompt: "Crée une identité visuelle complète pour une agence digitale spécialisée en IA.", color: "bg-blue-50 text-blue-600 group-hover:bg-blue-100" },
    ...
    { icon: <ZapIcon className="h-4 w-4" />, title: "Palette de couleurs", prompt: "Propose 3 palettes de couleurs pour une marque tech premium orientée confiance.", color: "bg-cyan-50 text-cyan-600 group-hover:bg-cyan-100" },
  ],
};
...
export function WelcomeScreen({ onSuggestion }: WelcomeScreenProps) {
  ...
  // ─── INAYA — welcome screen premium ───
  if (agent.persona) {
    return (
      <div className="flex flex-col items-center justify-start min-h-[calc(100vh-180px)] px-4 py-8 select-none">
        <InayaWelcomeCard persona={agent.persona} onSuggestion={onSuggestion} />
  ...
  const suggestions = AGENT_SUGGESTIONS[selectedAgentId] ?? GENERAL_SUGGESTIONS;
    ...
    <div className="flex flex-col items-center justify-start min-h-[calc(100vh-180px)] px-6 py-8 select-none">
      ...
      <div className="flex flex-col items-center gap-4 mb-8">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-sm text-center">{agent.description}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-2xl mb-8">
        {suggestions.map((s, i) => (
          ...
          >
            <div className={`flex-shrink-0 h-8 w-8 rounded-xl flex items-center justify-center transition-colors ${s.color}`}>{s.icon}</div>
